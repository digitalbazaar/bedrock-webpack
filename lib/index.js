...
bedrock.events.on('bedrock-cli.init', () => {
  bedrock.program
    ...
    .option('--webpack-symlink-watch <mode>',
      'Dereference node_module symlinks in watch mode' +
      ' (none, top, all).',
      /^(none|top|all)$/i, 'top')
...
export async function bundle(options = {}) {
  ...
  // FIXME: remove custom code when watch support handles symlinks
  // scan for symlinks
  async function aliasSymlinks({
    root, depth, maxDepth
  }) {
    // find root node_modules dir symlinks and add as aliases
    const modDir = path.join(root, 'node_modules');
    // check if path exists
    try {
      await fs.promises.access(modDir, fs.constants.R_OK);
    } catch(e) {
      return;
    }
    // check current level
    ...
    for(const dirent of modDirents) {
      if(dirent.isSymbolicLink()) {
        const from = path.join(root, 'node_modules', dirent.name);
    ...
    if(depth < maxDepth) {
      for(const dirent of modDirents) {
        if(dirent.isDirectory() || dirent.isSymbolicLink()) {
          await aliasSymlinks({
            root: path.join(root, 'node_modules', dirent.name),
            depth: depth + 1,
            maxDepth
          });
        }
  ...
  // scan top level
  if(command.webpackSymlinkWatch === 'top' ||
    // support old 'true' mode
    command.webpackSymlinkWatch === 'true') {
    await aliasSymlinks({
  ...
  // scan entire tree
  if(command.webpackSymlinkWatch === 'all') {
    await aliasSymlinks({
      root: appRoot.toString(),
      depth: 1,
