...
export async function bundle(options = {}) {
  ...
  const optimizeCss = isProduction && command.webpackOptimizeCss === 'true';
  ...
  // must extract CSS if `optimizeCss` is true
  if(optimizeCss || command.webpackExtractCss === 'true') {
  ...
  if(optimizeCss || command.webpackOptimizeJs === 'true') {
    const minimizer = [];
    if(optimizeJS) {
      const terserOptions = {
        parallel: true,
